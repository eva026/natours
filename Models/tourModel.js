...
const tourSchema = new mongoose.Schema(
  {
    name: {
      ...
      // validate: [validator.isAlpha, 'A tour name must be characters'],
    },
    slug: String,
    duration: {
      type: Number,
    ...
    priceDiscount: {
      ...
      validate: {
        ...
        message: 'PriceDiscount must be lower than price',
      },
    },
    summary: {
      type: String,
...
tourSchema.pre('aggregate', function (next) {
  // Check if the first in pipeline is $geoNear(Object.keys returns an array)
  const first = Object.keys(this.pipeline()[0])[0];
  if (first === '$geoNear') {
  ...
  // Add $match to the beginning of the pipeline
